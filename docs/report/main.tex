...
Архитектура интерпретатора — конвейер из четырёх компонентов: лексер → парсер → AST → интерпретатор. Дополнительно реализована REPL-оболочка.
...
Интерпретатор реализует \textbf{call-by-value} (энергичные вычисления).

\textbf{Представление значений (тип \texttt{value}):}
\begin{itemize}
    \item VInt, VBool — простые значения;
    \item VClosure (param, body, env) — замыкание для обычных функций;
    \item VRecClosure (name, param, body, env) — рекурсивное замыкание для let rec;
    \item VUninitialized — защита от преждевременного доступа.
\end{itemize}

Окружение — ассоциативный список \texttt{env = (string * value) list}.

\textbf{Монадическая обработка ошибок:} все функции возвращают \texttt{(value, string) Result.t}; оператор \texttt{let* = Result.bind} обеспечивает автоматическую передачу ошибки наверх.

\textbf{Ключевые конструкции:}
\begin{itemize}
    \item \textbf{Let:} вычисляется e1, результат связывается с x, затем вычисляется e2.
    \item \textbf{Let rec:} создаётся рекурсивное замыкание VRecClosure, ссылающееся само на себя через окружение.
    \item \textbf{Аппликация:} для VClosure — тело вычисляется в расширенном окружении; для VRecClosure — дополнительно добавляется связывание имени функции.
    \item \textbf{If:} cond = 0 или false → e2, иначе e1.
    \item \textbf{Бинарные/унарные операции:} типизированные, с проверкой деления на ноль.
\end{itemize}

\subsection{REPL-оболочка}

REPL поддерживает ввод выражений с немедленным вычислением, мета-команды (:h, :q, :c), сохранение окружения между вводами, обработку Ctrl+D.
...
\begin{itemize}
    ...
    \item Реализован интерпретатор с замыканиями, let rec (рекурсивные замыкания), монадической обработкой ошибок.
    \item Создана трёхуровневая система тестирования (Alcotest, QCheck, cram); покрытие 83\%.
    \item Настроена CI/CD на GitHub Actions.
\end{itemize}

\textbf{Теоретическая значимость:} демонстрация применения конечных автоматов, рекурсивного спуска, FIRST-множеств, операционной семантики с окружением, замыканий и монадической обработки ошибок.

\textbf{Ограничения:} динамическая типизация, отсутствие оптимизации хвостовой рекурсии, ограниченная диагностика ошибок, фиксированный набор типов (только int и bool).
...
\begin{thebibliography}{9}
    ...
    \bibitem{ocaml} Minsky Y., Madhavapeddy A., Hickey J. {\itshape Real World OCaml: Functional Programming for the Masses}. 2nd ed. Cambridge University Press, 2022. 530~p.
    \bibitem{appel} Appel A.W. {\itshape Modern Compiler Implementation in ML}. Cambridge University Press, 1998. 548~p.
    \bibitem{lambda} Барендрегт Х. {\itshape Лямбда-исчисление: его синтаксис и семантика}. М.: Мир, 1985. 606~с.
\end{thebibliography}
